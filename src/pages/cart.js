...
export default function Cart() {
    ...
    const {data: cartItems, isLoading: cartItemsLoading, refetch: refetchFoodItems} = useQuery('cart', async () => await fetch(`${API_URL}/cart/${userId}`,{
        headers:{
			authorization:localStorage.getItem("token")
		}
    }).then((res) =>  res.json()),{enabled: true });
    ...
    const updateCart = (id,action) => {
        console.log({id},localStorage.getItem('user'),'add',getQuantity(id,cartItems.items).id)
        ...
        cartItemsLoading ? <div>Loading...</div> : <div><div className="food-container">
			{
                ...
                // setQuantity(quantity)
                    ...
                    <button id={id} onClick={(e)=>{updateCart(e.target.id,'add')}} >+</button>
                    <p>{quantity}</p>
                    <button id={id} onClick={(e)=>{updateCart(e.target.id,'sub')}} >-</button>
